...
test, extend, and release Hivelore.** Reducing the bus-factor is an explicit goal: if you can follow this
...
- `hivelore eval` runs the retrieval + sensor quality gate; CI fails on a regression.
...
2. **Bump the version only if shippable code changed** (`@hivelore/core`/`cli`/`mcp`/`embeddings`);
