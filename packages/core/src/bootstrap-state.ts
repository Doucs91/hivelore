 ...
 * corpus itself. The very first agent on a fresh `haive init` faces an empty knowledge layer; this
 ...
 * mentions "auto-generated by `haive init`" in prose (e.g. a glossary line) is NOT a template.
 */
export function isTemplateProjectContext(raw: string): boolean {
  if (raw.includes("TODO — high-level overview")) return true;
  if (raw.includes("Generated by `haive init`")) return true;
  if (/Auto-generated by `haive init/i.test(raw) && (raw.match(/TODO —/g)?.length ?? 0) >= 2) return true;
  return false;
}
...
export function assessBootstrapState(input: BootstrapStateInput): BootstrapAssessment {
  ...
  if (input.codeFiles.length === 0) {
    gaps.push({
      kind: "index-code",
      detail: "No code-map found, so hAIve cannot verify per-area knowledge coverage.",
      items: [],
      fix: "Run `haive index code`, then re-run the bootstrap.",
    });
  }
