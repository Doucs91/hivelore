...
> ✅ **FIXED in v0.52.1.** `installGitEnforcement` now regenerates each hook via
> `buildHookFileContent` / `stripHiveloreHookBlock`: it strips any Hivelore-owned block (current OR
> legacy `# hAIve …` shape, incl. a previously-appended duplicate) and rewrites a single clean block;
> a foreign husky/custom hook is preserved with our block appended. Regeneration is a fixed point.
> Kept as history/regression context.
